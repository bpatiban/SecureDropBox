...
def main():
	...
	# Get keys in bucket
	mybucket = conn.get_bucket('minabucket')
	...
	for files in all_filenames: 
        	 ...
        	 if (files.startswith(sel_file) and files.endswith("R25")):
        	               selected_files.insert(j+1,files)
                   
        	 elif (files.startswith(sel_file) and files.endswith("R37")):
        	               selected_files.insert(j+1,files)
                       
        	 elif (files.startswith(sel_file) and files.endswith("R19")):
		 	      selected_files.insert(j+1,files)
	...
	for item in selected_files:
  		if not (item.startswith(".") or item.endswith("~")):
     			...
     			key.get_contents_to_filename('/home/jui/MagicBox/' + item) 
