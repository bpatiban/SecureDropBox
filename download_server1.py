...
def main():
	...
	for files in all_filenames: 
        	 ...
        	 elif (files.startswith(sel_file) and files.endswith("R67")):
        	               selected_files.insert(j+1,files)
                       
        	 elif (files.startswith(sel_file) and files.endswith("R38")):
		 	      selected_files.insert(j+1,files)
	...
	for item in selected_files:
  		if not (item.startswith(".") or item.endswith("~")):
     			k.key = item
     			k.get_contents_to_filename('/home/jui/MagicBox/' + item) 
