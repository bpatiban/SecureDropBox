...
def main():
	...
	for files in os.listdir("."):
		#for i in range(3):
				 ...
				 L_getfiles.insert(i+1,files)
               		if (files.startswith(sel_file) and files.endswith("R45")):
			         L_getfiles.insert(i+1,files)
               		if (files.startswith(sel_file) and files.endswith("R26")):
				 L_getfiles.insert(i+1,files)
	...
	if not files_to_upload:
	   fo.write("empty -- for upload - folder is in sunc \n")
	else: 
 
	   for item in files_to_upload:
  		if not (item.startswith(".") or item.endswith("~")):
   			k.key = item
   			s = "/home/jui/cloud/" + item
